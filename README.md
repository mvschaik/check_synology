...
This plugin uses ```python3``` & ```easysnmp``` with ```MD5``` + ```AES``` to check a lot of different values on your Synology DiskStation.

This check plugin needs ```easysnmp``` to be installed on your system. You can install it with: ```pip3 install easysnmp```
